$(function(){
	$('.logo').click(function(){
		alert($(".question .pic input").val());
		
	});
	$('body').on("selectstart", function(ev){ return false;ev=ev||window.event;ev.preventDefault(); });
	(function(){
		var i=145;
		var fl=0;
		var j=35;
		...
		setInterval(function(){
			$('body').css("background","radial-gradient(circle at 75% 0%,rgba("+(j+100)+","+(k+100)+","+(i+100)+","+0.8*h+") 0%,rgba("+j+","+k+","+i+","+h+") 100%)");
	...
	$("#question").validate({
		...
		submitHandler:function(form){
			...
			function checkpicType(){
				...
				var picArr=['jpg','jpeg','JPG','JPEG','png','PNG','GIF','gif'];
